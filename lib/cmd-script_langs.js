	...
	var xres = {
		...
		write: function(chunk) {
			...
			for(; i < l; i++)  {
				...
				if (!lang_old[x]) {
					if (options.all == 'unicode' &&  !/[^\x00-\xff]/i.test(x) ) {
						continue;
					};

					if (options.all == 'ru' &&  !/[аА-яЯёЁ]/i.test(x) ) {
						continue;
					};
				};
				...
				langsFor.forEach(function(lang) {
					...
					if (key == 'key' && autokey && typeof value === 'string') {
						if (value[0] === '.') {
							value = autokey + value;
						};
					};
			...
			if (!options.pure && typeof lang_old === 'object') { // ишем протухшие значения
				for(x in lang_old) {
					if (lang_old[x] && !lang_new[x]) {
						(lang_new[x] = lang_old[x]).deleted = true;
					};
				};
			};
