...
function fileLoad(url, options, end) {
	...
	if (src.protocol === 'file:') {
		FS.readFile((PATH.sep === '\\' ? (src.path||'').substr(1) : src.path), function (err, data) {
			if (err) {
	...
	var auth = options.authorization ? accessDomain(src.protocol, src.host, false) ? options.authorization : null : null;
	if (auth) query.headers.authorization = auth;
