...
function ServerHendler(req, res) {
	...
	if (/^\/file\/./.test(q.pathname)) {
		script_prox(req, res, q);
		return;
	};
...
function script_prox(req, res, q) {
	...
	*/
	...
	if (qm) {
		qm[3] = qm[3] ? (qm[3][0] === '-' ? qm[3].replace('-', 'module') : qm[3]) : 'module';
	...
	var zA = '__MODULE(\''+qm[2]+'\', function(global,'+qm[3]+',__zAgS_,__zTr_){\'use strict\';__zAgS_(function(){return[global,'+qm[3]+']});';
	var zB = '\n});';
