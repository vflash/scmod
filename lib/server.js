...
var fileProx = require('./file-prox.js');
...
function ServerHendler(req, res) {
    ...
    fileLoad(url_replace, {authorization: req.headers['authorization'], X_Forwarded_For: req.X_Forwarded_For}
...
function prox_script(req, res, q) {
    var qm = String(q.path).match(/\/prox-js\/(-|\+)\/(\w+)\/([^\/]+)\/(https?)\/(.+)/) || false;
    /*
    qm[1] - lang
    qm[2] - module ID
    qm[3] - module vars
    ...
    */
    ...
    var zA = '__MODULE(\''+moduleID+'\', function(global,'+moduleVARS+',__zAgS_){\'use strict\';__zAgS_(function(){return[global,'+moduleVARS+']});';
    var zB = '\n});';


    if (qm[1] === '+') {
        proxSBoxTranslate(moduleID, file, req, res, zA, zB);
    } else {
        fileProx(file, req, res, true
            , zA
            , zB
        );
    };
...
function prox_jsTranslate(req, res, q) {
    var qm = String(q.path).match(/\/prox-tr\/(\w+)\/(https?)\/(.+)/) || false;
    /*
    qm[1] - module ID
    qm[2] - src protocol
    qm[3] - src
    */
    ...
    proxSBoxTranslate(moduleID, file, req, res);
};


var regJsBlock = genRegExp(['str1', 'str2', 'comment_mu', 'comment_mi', 'regx'], 'g');

function proxSBoxTranslate(moduleID, file, req, res, zA, zB) {
    var BUFF = [];
    ...
    var ok = false, xres = {
        ...
        write: function(chunk) {
            ...
            if (chunk) BUFF.push(chunk);

            var code = Buffer.concat(BUFF).toString();
            BUFF.length = 0;

            code = code.replace(regJsBlock
                , function(x) {
    ...
    fileProx(file, req, xres, true, zA, zB);
};
