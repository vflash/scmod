...
function modscript(log, ureq, url, end) {
    ...
    smod(log||function(){}, ureq, url, function(status
        ...
        while(x = global_scripts[i++]) {
            ...
            if (x.type === 'common-js') {
                file.url = qhost + '/prox/common-js/' + x.moduleID + '/-/' + (x.src).replace(/^(https?):\/\//, '$1/');
                continue;
            };

            if (x.type === 'text') {
                file.url = qhost + '/prox/text/' + x.moduleID + '/-/' + (x.src).replace(/^(https?):\/\//, '$1/');
                continue;
            };

            if (x.type === 'json') {
                file.url = qhost + '/prox/json/' + x.moduleID + '/-/' + (x.src).replace(/^(https?):\/\//, '$1/');
                continue;
            };
...
function smod(log, ureq, startURL, end_compite) {
    ...
    var xAuthorization = ureq.headers['authorization'];
    var xForwardedFor = ureq.X_Forwarded_For
    var replaceHash = ureq.replaceHash || false;  // false - не указан , true - пустой
    var xStartHost = ureq.xStartHost;
    var modules = {};
    var files = [];
    var styles = [];
    var stop;
    ...
    get_module('module', xStartHost, startURL, [], function() {
        end_compite(true, modules, files, styles);
    });

    function get_module(type, host, url, modstack, end) {
        var virtmod = false, x;
        ...
        */
        ...
        load(url, loadOptions, function(status, result) {
            if (stop) return;
            ...
            result.get(['module'], function(status, res) {
                ...
                if (!replaceHash) {
                    replaceHash = jsonModule.replace ? genReplaceHash(xurl, jsonModule.replace, xStartHost) : true;
                };
        ...
        function mload(jsonModule) {
            ...
            if (moList = jsonModule.modules) {
                for (var name in moList) {
                    ...
                    if (!/^file:/.test(src)) {
                        //src = formatURL(xStartHost, xurl, src, replaceHash);
                        var qq = src;
                        var src_ = formatURL(host, xurl, src, replaceHash, true);
                        srcHost = src_.host;
                        src = src_.value;
                    } else {
                        src = 'badurl-a:' + src;
        ...
        function loadModule(mod, end) {
            ...
            var modStackID = mod.id + ' ' + mod.src;
            if (modstack.indexOf(modStackID) != -1) {
                stop = true;
            ...
            get_module(mod.type, mod.host || host, mod.src, modstack.concat(), function(status, id) {
                modules_loaded += 1;
        ...
        function pushFile(xmod, list) {
            ...
            for(i = 0, l = list.length; i < l; ++i) {
                ...
                if (!file.inc) {
                    if (!/^file:/.test(value)) {
                        value = formatURL(host, xurl, value, replaceHash);
                    } else {
                        value = 'badurl-a:' + value;
                    };
