            ...
            if (a = mod_json.scripts || mod_json.files) {
                for(i=0, l = a.length; i<l; i+=1) {
                    ...
                    if (v = a[i].trim() ) {
                        ...
                        if (!js_inc) {
                            v = formatURL(xurl, (/^file:/.test(v) ? 'badurl-b:' + v : v), replaceHash);
            ...
            if (a = mod_json.styles) {
                for(i=0, l = a.length; i<l; i+=1) {
                    ...
                    if (x = a[i].trim()) {
                        styles.push({
                            moduleID: xmod.id,
                            src: formatURL(xurl, (/^file:/.test(x) ? 'badurl-c:'+x : x), replaceHash)
                        });
                    };
                };
            };
...
function pushStyle(styles, xmod, xurl, replaceHash, ureq, a) {
    ...
    for(; i<l; i+=1) {
        ...
        if (v = a[i].trim()) {
            v = formatURL(xurl, (/^file:/.test(v) ? 'badurl-c:' + v : v), replaceHash);
            if (typeof v !== 'string') {
                continue;
            };
