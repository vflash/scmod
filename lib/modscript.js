...
function smod(log, ureq, startURL, end_compite) {
    ...
    function get_module(url, modstack, end) {
        ...
        function mload(mod_json) {
            ...
            if (mod_json.alias) {
                xmod.alias = ('' + mod_json.alias).trim();
            };

            pushFile(xmod, mod_json.stylesForce);

            if (x = mod_json.modules) {
                for (var i in x) {
                    var src = x[i], type = null;
                    ...
                    src = formatURL(xurl
                        , /^file:/.test(src) ? 'badurl-a:' + src : src
                        , replaceHash
                    );
        ...
        function complit() {
            ...
            pushFile(xmod, mod_json.scripts);
            pushFile(xmod, mod_json.styles);
            pushFile(xmod, mod_json.src);
        ...
        function pushFile(xmod, list) {
            ...
            for(i = 0, l = list.length; i < l; ++i) {
                ...
                if (!file.inc) {
                    ...
                    file.type = (/\.([^\.]+)$/.exec(value.replace(/\?.*$/, ''))||false)[1] || file.type;
               };
