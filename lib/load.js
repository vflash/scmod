...
var Result = require('./file-result.js');
...
function load(url, options, end) {
    ...
    if (cacheObj) {
        addHeaders('if-modified-since', cacheObj.LastModified);
        addHeaders('if-none-match', cacheObj.Etag);
    } else {
        addHeaders('if-modified-since', options.LastModified);
        addHeaders('if-none-match', options.Etag);
    };

    if (options.XForwardedFor) {
        addHeaders('x-forwarded-for', options.XForwardedFor);
    };
