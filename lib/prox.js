...
var rgxSassImport = regxMaster(/[:import:]/g, {
    import: /(?:^|\n)(\s*)@import\s+[^\s]+/
});



wrap['css'] = function(qm, result, end) {
    // import
...
wrap['js'] = function(qm, xfile, end) {
    // common-js -> import(wrap+translate)
    var isTranslate = /^\+/.test(qm.vars);
    var host = xfile.src.host;
    ...
    function wrap(data) {
        var mp = {}, ix = 0;
        var data = (data + '').replace(rgxCommonJS2My, function(s, cha, mUrl, type) {
            if (mUrl == null) {
                return s;
            };

            switch(type = type ? type.substr(1, arg1.length-2) : null) {
                case 'json':
                case 'text':
                    break;
                default:
            ...
            try {
                mUrl = JSON.parse(mUrl);
            } catch (e) {
                return s;
            };

            mUrl = formatURL(host, mUrl);

            return cha + 'requery(\'' + genUNIC(type, mUrl) + '\')';
        });
...
wrap['common-js'] = function(qm, xfile, end) {
    ...
    function wrap(data) {
        ...
        var data = (data + '').replace(rgxCommonJS2My, function(s, cha, arg1, arg2) {
            ...
            if (arg1[0] === '\'') {
                arg1 = '"' + arg1.substr(1, arg1.length-2).replace(/\\(.)|(")/g, '\\$1$2') + '"';
            };
            ...
            return cha + 'REQUERY_(' + JSON.stringify(arg1) + ']';
        });
